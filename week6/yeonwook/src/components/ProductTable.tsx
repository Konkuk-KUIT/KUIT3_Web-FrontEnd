...
const ProductTable: React.FC<Props> = ({
  ...
  const deleteProduct = (productDeleted: Products) => {
    setProducts((previousData: Products[]) => 
    previousData.filter((product)=>(product !== productDeleted)));
  };
          ...
          return (
            <Fragment key={productCategory.category}>
              ...
              {productCategory.products.map((product) => (
                <ProductRow key={product.id} product={product} setProducts={setProducts} deleteProduct={deleteProduct} />
              ))}
            </Fragment>
          );
        })}
